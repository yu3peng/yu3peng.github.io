...
由事件源触发的事件调用函数。函数和事件源之间存在n：m映射。每个事件源可用于调用多个函数，函数可由多个事件源触发。事件源可以映射到函数的特定版本或函数的别名，后者提供了更改函数和部署新版本的方法，而无需更改事件关联。事件源也可以定义为使用相同功能的不同版本，并定义应为每个版本分配多少流量。
...
以下列表描述了功能和无服务器运行时应基于当前技术水平满足的常见需求集：
...
- 一个事件可以触发按顺序或并行执行的多个功能

- “和/或事件组合”可以触发按顺序或并行或分支运行的m个函数

- 在工作流程的中间，可能会收到不同的事件或功能结果，这会触发分支到不同的功能
...
异步消息队列请求（发布/订阅），例如， RabbitMQ，AWS SNS，MQTT，电子邮件，对象（S3）更改，计划事件（如CRON作业）
